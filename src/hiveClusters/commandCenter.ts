...
export class CommandCenter extends HiveCluster {
	...
	visuals(coord: Coord): Coord {
		...
		if (this.storage) {
			...
			Visualizer.barGraph(_.sum(this.storage.store) / this.storage.store.getCapacity(),
								{x: boxX + 4, y: y, roomName: this.room.name}, 5);
			y += 1;
		}
		if (this.terminal) {
			Visualizer.text('Terminal', {x: boxX, y: y, roomName: this.room.name});
			Visualizer.barGraph(_.sum(this.terminal.store) / this.terminal.store.getCapacity(),
								{x: boxX + 4, y: y, roomName: this.room.name}, 5);
			y += 1;
		}
		return {x: x, y: y + .25};
