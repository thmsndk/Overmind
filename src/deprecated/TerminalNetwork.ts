...
export class TerminalNetwork /*implements ITerminalNetwork*/ {
	...
	private remainingRoomCapacity(room: Room): number {
		...
		if (room.storage) {
			remainingCapacity += room.storage.store.getCapacity() - room.storage.store.getUsedCapacity();
		}
		if (room.terminal) {
			remainingCapacity += room.terminal.store.getCapacity() - room.terminal.store.getUsedCapacity();
		}
			...
			if (state.type == 'out' || state.type == 'in/out') {
				if (terminal.cooldown == 0 && amount > targetAmount + tolerance) {
					...
					if (receiver) {
						...
						if (receiver.store.getCapacity() - receiver.store.getUsedCapacity() > sendAmount) {
